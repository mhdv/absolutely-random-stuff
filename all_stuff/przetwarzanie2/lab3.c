...
Środa 20.01.2016;
	...
	Program został podzielony na moduły, każda funkcja programu jest
	od teraz w osobnym "pliku.c".
	Z uwagi na wygodę kompilacji programu podzielonego na moduły, został
	dodany tzw. "makefile", który znacznie skraca czas potrzebny na
	skompilowanie programu.
  -Alternatywnie dla makefile, zostały napisane również dwa skrypty.
  Skrypt "gcc.sh" odpowiadający za kompilację programu.
  Skrypt "rm.sh" odpowiadający za usunięcie plików *.o oraz ich kompilację
  na nowo.
	Wszystkie zaimplementowane funkcje w programie działają poprawnie,
	program poprawnie wczytuje, zapisuje obrazki oraz wyświetla komunikaty
	o błędach i czynnościach, których nie może wykonać.
	Niestety z powodu braku czasu nie udało mi się dodać do programu
	potrzebnych funkcji z zadania "przetwarzanie obrazów 2".
